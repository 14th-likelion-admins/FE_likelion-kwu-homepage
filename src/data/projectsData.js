...
특히 링크, 게시글, 대화 로그 등 다양한 비정형 데이터를 실시간으로 처리, 가공하는 구조를 통해 개별 경험에 의존하던 범죄 대응 방식을 집단적, 시스템적 대응 방식으로 확장하고자 하였습니다.`,
    ...
    학교 주변 상권은 프랜차이즈 중심으로 소비가 몰리며 소상공인의 매출이 점차 줄어드는 반면, 대학생들은 절약의 필요성을 느끼면서도 이를 꾸준히 실천할 **동기부여가 부족**하다는 문제가 있습니다. 또한 기존 금융/가계부 앱은 소비 기록이나 프랜차이즈 위주의 쿠폰 제공에 머물러 **절약 습관과 지역 경제 문제를 연결**하지 못하고 있습니다.

    SLGM은 사용자의 소비 기록을 기반으로 **AI가 지출 패턴을 분석**해 개인의 금융 습관을 개선할 수 있도록 돕고, **절약/소비 챌린지**를 통해 포인트를 적립하여 **제휴 지역 매장 쿠폰**으로 전환할 수 있게 설계했습니다. 지정 금액 이상 지출이 발생하면 알림으로 경각심을 주고, 챌린지 성공 경험을 리워드로 연결해 “절약이 곧 지역 소비의 선순환”이 되도록 만듭니다.

    개인에게는 금전적 여유와 지속 가능한 절약 습관을, 소상공인에게는 매출 증대와 지역 경제 활력을 제공하는 **개인·지역 동반 성장 플랫폼**을 지향합니다.`,
        features: [
          '소비 기록 기반 AI 지출 패턴 분석 및 맞춤형 소비 리포트 제공',
          '절약/소비 챌린지 생성 및 참여(목표 금액·기간 기반)',
          '챌린지 성공 시 포인트 적립 및 리워드 히스토리 관리',
          '지정 금액 이상 결제 시 경고 알림으로 과소비 예방',
          '포인트를 지역 제휴 매장 쿠폰으로 전환/구매 및 사용',
          '지역 매장(소상공인) 제휴 등록 및 쿠폰/혜택 운영 기능',
          '학교 주변 상권 중심 추천(지역/카테고리 기반) 및 탐색',
          '정책/프로그램 연계(지자체·기업 파트너십) 확장 구조 고려',
          '개인정보·금융 데이터 보안 및 동의 기반 데이터 처리(규제 대응)',
        ],
      },
    },
    ...
    외국인 유학생은 계약서가 대부분 한국어로 작성되어 있어 중요한 조건을 정확히 이해하기 어렵고, 전세/월세/관리비/옵션 등 한국 특유의 주거 개념에 익숙하지 않아 불리한 계약이나 오해가 발생하기도 합니다. 또한 입주 후 고장 신고, 공지 확인, 요청 전달 등 생활 단계에서도 집주인·중개인과의 소통이 어려워 기본적인 주거권 행사조차 제한되는 경우가 있습니다.

    Room-Mate는 **방 찾기 → 계약 → 입주 후 소통**까지 전 과정을 하나의 흐름으로 연결합니다. 계약서는 OCR 기반 문서 이해와 AI 분석을 통해 **핵심 조건 요약 및 위험 조항 탐지**를 제공하고, 어려운 용어는 쉬운 설명과 다국어 번역으로 풀어줍니다. 입주 후에는 **실시간 양방향 번역 채팅**과 문제 상황별 가이드를 통해 언어 걱정 없이 빠르게 해결할 수 있도록 돕습니다.`,
        features: [
          '방 찾기: 지역/월세 등 조건 기반 필터링 + 쉬운 한국어/다국어 정보 제공',
          '방 찾기: 금액을 줄임 없이 전체 숫자로 표기해 오해 최소화',
          '방 찾기: 지도 기반 위치 비교(매물 위치와 주요 장소 마커 동시 표시)',
          '방 찾기: 외국인 관점 핵심 항목 선별로 매물 상세 정보 간결화',
          '중개 연결: 외국어 가능 중개사에게 전화/문자 연결 지원',
          '계약 단계: 계약서 업로드(OCR) → 핵심 조건 요약 + 중요 항목 하이라이트',
          '계약 단계: 위약금/숨은 비용 등 위험 조항 자동 표시 및 주의 환기',
          '계약 단계: 전세/월세/관리비 등 어려운 개념을 쉬운 설명 + 아이콘 UI로 시각화',
          '법률 지원: 불리한 조항/이해 어려움 발생 시 법률 상담(문자 등) 연결',
          '입주 후 소통: 집주인·세입자 간 실시간 양방향 번역 채팅',
          '입주 후 소통: 공지/요청 히스토리 관리 및 집 정보/집주인 정보 빠른 확인',
          '문제 해결: 규칙 기반 분류 + 유사 사례 기반 안내(CBR)로 상황별 해결 절차 제공',
        ],
      },
    },
